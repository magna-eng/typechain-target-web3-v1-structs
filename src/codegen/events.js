...
function codegenOutputTypesForEvents(outputs) {
    return `{
    ${outputs.map((t) => t.name && `${t.name}: ${(0, types_1.codegenOutputType)({ useStructs: true }, t.type)}, `).join('')}
    ${outputs.map((t, i) => `${i}: ${(0, types_1.codegenOutputType)({ useStructs: true }, t.type)}`).join(', ')}
  }`;
}
